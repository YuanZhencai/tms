...
function DisabledCSS(){
	//**************
	// 对Radio控件选中选项添加样式 add by yanchangjing on 2013-7-3
	$("input[checked='checked']").parent().parent().parent().next().children().css("color","#666666");
	//**************
...
function numeralInt(obj,len,defVal,maxVal){
	...
	obj.bind("keypress",function(event) {
		if(obj.val().length < len){
			...
			return (k >= 48 && k <= 57)||k==8||k==46||k==37||k==39;
		}
    });
...
function toDecimal(inputId,n){
	...
	thisObj.keypress(function(event){
		var k=window.event?event.keyCode:event.which;  
		return (k >= 48 && k <= 57)||k==8||k==46||k==45||k==37||k==39;
	}).blur(function(){
		getParseFloat(thisObj,n);
	}).bind("paste", function() {
		getParseFloat(thisObj,n);
