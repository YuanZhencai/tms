 ...
 */
@Stateless
public class PatchProcessSubedService implements Serializable{
    ...
    private static final Log log = LogFactory.getLog(PatchProcessSubedService.class);
		...
		try {
			...
			for(VWWorkObject wob : wobs){
				...
				if(this.checkNoFilter(conditionMap ,pd)){
					...
					vo.setOriginator(wob.getOriginator());
...
//		//1.创建人检查
	...
	public List<ProcessInstanceVo> getTerminatedProcessInstanceVos(Map<String ,Object> conditionMap) throws ServiceException{
		...
		List<ProcessInstanceVo> pivos = new ArrayList<ProcessInstanceVo>();
		try {
		    // 查询条件，公司名称，结束时间from，结束时间to
            String companyId = (String) conditionMap.get("companyId");
            Date endFromTime = (Date)conditionMap.get("endFromTime");
            Date endToTime = (Date)conditionMap.get("endToTime");
			...
			for(VWLogElement le : les){
				...
				//结束时间，查询条件过滤
				Date processEndDate = le.getTimeStamp();
				if(processEndDate == null){
					continue;
				}
				
				if(endFromTime != null && endToTime != null){		
					if( processEndDate.before(endFromTime) || processEndDate.after(endToTime)){
						continue;
					}
				}
				...
				if(this.checkNoFilter(conditionMap ,pd)){
					...
					vo.setOriginator(peManager.vwGetUserNameById((Integer)le.getDataField("F_Originator").getValue()));
			...
			System.out.println("getTerminatedProcessInstanceVos for les  costTime:"+(costTime) +"秒");
		} catch (Exception e) {
