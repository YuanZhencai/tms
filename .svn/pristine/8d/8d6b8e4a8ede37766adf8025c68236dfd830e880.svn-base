...
public class RegisterCaptialBean extends FileUpload<ProcRegiCapital> {
    ...
    public void init() {
        ...
        if (audit != null && workclassNumber != null) {
            ...
            fillData(wcnum, flag);
            initdata(true);
            registerDate = DateUtil.dateToStrShort(DateUtil.dateToDateTime(getInstance().getApplyDate()));
            if (!"申请".equals(title)) {
    ...
    public void fillData(String workclassNumber, String flag) {
        if (workclassNumber != null) {
            try {
                ProcRegiCapital preg = this.registerCaptialService.findProcRegiCaptial(workclassNumber);
                ...
                List<ShareHolder> list = shareHolderService.findShareHolderListByCp(companyId);
                if (!list.isEmpty()) {
                    shareHolderSelect.clear();
                    for (ShareHolder sh : list) {
                        shareHolderSelect.add(new SelectItem(sh.getId(), sh.getShareHolderName()));
                    }
                }
                ...
                if ("集团项目经理审批".equals(title)) {
                    ...
                    if (preg.getThisFondsCu() != null && !"".equals(preg.getThisFondsCu())) {
    ...
    /**
     * 
     * <p>Description: 发起注册资本金申请</p>
     * @return
     */
    public String applyRegister() {
        try {
        	//如果是工程类表单做附件检查
        	if(getInstance().getApplyType()!=null && "Y".equals(getInstance().getApplyType())){
        		List files = this.getProcessFileList();
        		if(files==null || files.size()==0){
        			MessageUtils.addErrorMessage("msg", MessageUtils.getMessage("msg_fileUpload_require"));
        			return null;
        		}
        	}

            String workflowClassName = ProcessXmlUtil.getProcessAttribute("id", "RegiCapital", "className");
            String workClassnumber = registerCaptialService.vwCreateProcessInstance(workflowClassName, "注册资本金", getInstance()
                    .getPeMemo(), this.getDocmentIdList());
            // 保存流程附件
            ...
            getInstance().setApplyDate(DateUtil.strToDate(registerDate, "yyyy-MM-dd").toDate());
            getInstance().setProcInstId(workClassnumber);
            registerCaptialService.saveRegisCaptial(this.getInstance());
            MessageUtils.addSuccessMessage("doneMsg", MessageUtils.getMessage("process_new_success", processUtilMapService.getTmsIdByFnId(workClassnumber)));
    ...
    public String reApplyRegister() {
        try {
            ...
            registerCaptialService.vwReplayRegister(getInstance().getPeMemo(), workClassNum, this.getDocmentIdList());
            // 保存流程附件
            ...
            MessageUtils.addSuccessMessage("donMsg", MessageUtils.getMessage("process_new_success_1", processUtilMapService.getTmsIdByFnId(workClassNum)));
            //9.3处理下一个任务
    ...
    public String approveRegisterCaptial() {
        try {
            ...
            String processFlow = "";
            double unregistered = this.shareHolderService.calculationUnregistered(companyId);
            // 参数Map
            Map<String, Object> paramMap = Maps.newHashMap();
            paramMap.put("_Pass", flag);
            paramMap.put("_Arrived", unregistered);
            String inputable = ProcessXmlUtil.findStepProperty("id", "RegiCapital", title, "inputable");
            // 审批节点是否需要更新实体
            if (inputable != null && !"".equals(inputable)) {
                entityService.update(getInstance());
            }
            if (flag) {
                processFlow = ProcessXmlUtil.findStepProperty("id", "RegiCapital", title, "passMemo");
                MessageUtils.addSuccessMessage("doneMsg", MessageUtils.getMessage("approve_pass_success", processUtilMapService.getTmsIdByFnId(workObjNum)));
            } else {
                processFlow = ProcessXmlUtil.findStepProperty("id", "RegiCapital", title, "nopassMemo");
                MessageUtils.addSuccessMessage("doneMsg", MessageUtils.getMessage("approve_nopass_success", processUtilMapService.getTmsIdByFnId(workObjNum)));
            }
            registerCaptialService.vwDisposeTask(getInstance().getProcInstId(), paramMap,
...
//        processDetailList = processWaitService.findProcessDetialList(filter, substitutionVars);
    ...
    public void companyEnName() {
        ...
        // 得到股东信息
        List<ShareHolder> shareHolderList = shareHolderService.findShareHolderListByCp(com.getId());
        shareHolderModel = new CustomPageModel<ShareHolder>(shareHolderList, false);
    ...
    public boolean checkInputable(String fieldName) {
        if (inputableFields != null && !inputableFields.isEmpty() && inputableFields.contains(fieldName)) { return true; }
        return false;
    }
    ...
    private void getInputable(String stepName) {
        inputableFields = ProcessXmlUtil.getInputableDatas("RegiCapital", stepName);
    }
