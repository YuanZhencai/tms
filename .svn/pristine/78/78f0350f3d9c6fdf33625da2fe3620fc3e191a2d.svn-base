...
public class DebtBorrowBean extends FileUpload<ProcDebtBorrow>{
    ...
    public void initOp(){
    	...
    	if(op!=null && !"".equals(op)){
    		...
    		if("approve".equals(op)){
    			...
    			initCompany(true);
    			//集团审批相关信息，设置默认值
    			if(checkInputable("corpAudit") && (procDebtBorrow.getCorpAudit()==null || procDebtBorrow.getCorpAudit().equals(0d)) ){
    				procDebtBorrow.setCorpAudit(procDebtBorrow.getThisShBorrow());
    			}
    			if(checkInputable("corpAuditLi") && procDebtBorrow.getCorpAuditLis()==null){
    				procDebtBorrow.setCorpAuditLis(procDebtBorrow.getThisShBorrowLis());
    			}
    			if(checkInputable("corpAuditLi") &&procDebtBorrow.getCorpAuditLie()==null){
    				procDebtBorrow.setCorpAuditLie(procDebtBorrow.getThisShBorrowLie());
    			}
    			if(checkInputable("corpAuditRa") && (procDebtBorrow.getCorpAuditRa()==null || procDebtBorrow.getCorpAuditRa().equals(0d)) ){
    				procDebtBorrow.setCorpAuditRa(procDebtBorrow.getThisShBorrowRa());
    			}
    			if(checkInputable("corpAuditCu") && (procDebtBorrow.getCorpAuditCu()==null || "".equals(procDebtBorrow.getCorpAuditCu())) ){
    				procDebtBorrow.setCorpAuditCu(procDebtBorrow.getThisShBorrowCu());
    			}
    			//非重新申请页面设置“处理意见”默认值
                if(!"申请".equals(stepName)){
                	procDebtBorrow.setPeMemo("同意");
                }
                typeDesc = "Z".equals(procDebtBorrow.getThisShBorrowSe()) ? "展期申请" : "外债申请";
    		}
    	}else{
     ...
     * 借款期限起始时间 提醒
     */
    public boolean checkLimitDate(String idName,boolean required,boolean showWarn){
    	...
    	if(idName.equals("thisShBorrowLis")){
    		...
    		if(required && limitStart==null){
    			MessageUtils.addErrorMessage("msg", MessageUtils.getMessage("msg_required_thisShBorrowLis")+"!");
    			return false;
    		}
    		if(limitStart!=null){
    			String limitStr = sdf.format(limitStart);
    			if(limitStr.compareTo(nowStr)<0 && showWarn){
    				MessageUtils.addWarnMessage("msg", MessageUtils.getMessage("errMsg_thisShBorrowLis")+"!");
    				return true;
    			}
    		}
    	}
    ...
    private void procDebtBorrowFilter(){
    	...
    	if(company!=null && company.getId()!=null){
    		List<ShareHolder> shs = procDebtBorrow.getCompany().getShareHolders();
    ...
    public String createProcInstance(){
    	if(procDebtBorrow.getAvailbBebt() < 0){
    		MessageUtils.addErrorMessage("msg", MessageUtils.getMessage("errMsg_availbBebt"));
    		return null;
    	}
    	...
    	Company company = entityService.find(Company.class,procDebtBorrow.getCompany().getId());
    	procDebtBorrow.setCompany(company);

    	try {
    		String procInstId = debtBorrowService.createProcInstance(procDebtBorrow);
    		// 保存流程附件
    ...
    public void findProcInstance(){
    	procDebtBorrow = debtBorrowService.findProcInstanceByProcInstId(procInstId);
    ...
    private void getInputable(){
    	inputableFields = ProcessXmlUtil.getInputableDatas("DebtBorrow", stepName);
    }
    ...
    public String doApprove(){
    	if(!this.checkLimitDate("corpAuditLis",this.checkInputable("corpAuditLi") ? true : false , false)){
    		return null;
    	}
