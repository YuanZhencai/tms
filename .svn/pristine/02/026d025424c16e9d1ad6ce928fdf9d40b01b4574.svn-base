package com.wcs.tms.service.process.common.patch.view;
...
import com.wcs.tms.service.process.common.patch.service.PatchProcessSubedService;
...
@ManagedBean
@ViewScoped
public class PatchProcessSubedBean implements Serializable {

	private static final long serialVersionUID = 1L;
	
	private static final Log log = LogFactory.getLog(PatchProcessSubedBean.class);

	@Inject
	EntityService entityService;
	@Inject
	PatchProcessSubedService processSubedService;
	@Inject
	...
	public void clearAllTMPData(){
		//先清理
		int rows = patchMainService.cleanTMPData();
		MessageUtils.addSuccessMessage("doneMsg", "成功清理掉PATCH_SUBED_PROCESS表中所有数据，共"+rows+"条。");
	}
	
	/**
	 * 导出数据到PATCH表
	 */
	public void exportData2TMP(){
		try {
			if (isCopUser == true) {
			...
			//保存至临时表
			for (ProcessInstanceVo vo :processInstanceVos) {
				patchMainService.saveData2TMP(vo);				
			}
			...
			System.out.println("写入patch表的时间  costTime:"+(costTime) +"秒");
			MessageUtils.addSuccessMessage("doneMsg", "数据成功导入到PATCH_SUBED_PROCESS表中，共"+processInstanceVos.size()+"条。");
